...
export default class SIFecha_MY_Alert extends Component<typeProps> {
    sdate
    scroll
    refItens
    state
    ...
    selectIten(key, y) {
        if (this.refItens[key][y]) {
            ...
            var sdate = new SDate(this.state.select.year + "-" + SDate.formatCero(this.state.select.month) + "-01", "yyyy-MM-dd");
            if (sdate.isValid()) {
    ...
    getListaKey = (key) => {
        ...
        return arr.map((obj) => {
            if (obj.type == "month" && this.state.select.year && this.state.select.month) {
                var fecha = this.state.select.year + "-" + this.state.select.month + "-01"
